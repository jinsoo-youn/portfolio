...
또한 kubernetes 인증&인가와 Admission에 대해 정확히 이해하고 있으며, 외부 인증서버(keycloak, google 등)를 통한 인증 환경을 구축할 수 있는 능력을 보유하고 있습니다. 
현재 kubernetes 클러스터의 보안 강화를 위해 opa를 적극 도입하기위해 연구 중에 있습니다. 

# Introduction. 

제 자신을 한 단어로 “왜?” 입니다.   
  의미와 진실을 추구하기에 탐구심이 많으며, 이해하지 못하면 행동하지도 못합니다. 이를 통해 저의 특징을 소개해보고자 합니다.

특징:
- 모든 정신력과 시간을 집중해 이해하기 위해 노력한다. 본질을 이해 했기에 관련된 일에 대해 직관을 가지며 맞는 방향으로 빠르게 일처리가 가능하다.
- 의미와 진실이 중요하기에 맡은 일에 대해 신중하게 판단하려 노력한다.
- 아는 것을 안다하고, 모르는 것을 모른다고한다. 모르는 것은 알기위해 타인에게 고개를 숙이는 일을 부끄러워하지 않는다.
- 무엇이 중요한지 알기에 무엇을 해야하는지 스스로 찾아 일을 한다.
- 문제를 해결하는데 있어 중요한 것이 협력임을 잘 이해하고 있기에 상대방에게 예의를 갖추고 겸손한 자세로 협력한다.
